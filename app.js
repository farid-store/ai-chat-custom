...
let state = {
    ...
    tableSortKey: 'monthYear', // Kunci pengurutan default
    tableSortDir: 'asc',       // Arah pengurutan default
};

// --- 3. FUNGSI UTILITAS UI ---

const updateLucideIcons = () => {
    // Fungsi ini kini dipanggil lebih sering untuk memperbarui ikon yang ditambahkan secara dinamis
...
const createMessageBubble = (role, text) => {
    ...
    // Tambahkan delay kecil untuk efek fade-in
    setTimeout(() => {
...
// --- 4. DATA TABLE LOGIC (NEW UX) ---

const renderHistoricalData = (dataToRender) => {
    // Gunakan data yang sudah di-filter/di-sort
    const data = dataToRender || rawData;
...
const sortAndRenderTable = (dataToSort = rawData) => {
    const { tableSortKey, tableSortDir } = state;
    
    // Logika pengurutan sederhana (hanya untuk string)
...
const handleSortClick = (key) => {
    ...
    filterTable(); // Sort data yang sudah difilter
};
...
const updateFormState = () => {
    ...
    if (!apiKeyPresent) {
         ...
         // Tampilkan pesan awal hanya jika belum ada pesan
...
const toggleChatMaximize = () => {
    ...
    if (state.isChatMaximized) {
        ...
        elements.chatContainer.classList.replace('h-[80vh]', 'h-[90vh]'); // Sedikit lebih tinggi
    ...
    updateLucideIcons();
    // Scroll untuk memastikan posisi chat tetap terlihat
    scrollToBottom();
};
